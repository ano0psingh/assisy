      ...
      <div className={`relative rounded-2xl max-w-sm w-full overflow-hidden ${isDark ? 'bg-slate-900 border border-white/10' : 'bg-white border border-slate-200'}`}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSkip}
              className={`flex-1 py-2.5 rounded-lg text-sm font-medium ${isDark ? 'bg-white/5 text-gray-400 hover:bg-white/10' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            >
              Skip
            </button>
          </div>
