  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative rounded-2xl shadow-elevated w-full max-w-lg max-h-[80vh] flex flex-col animate-slide-up overflow-hidden ${
        ...
        {/* Header */}
        <div className={`flex items-center justify-between px-6 py-4 border-b flex-shrink-0 ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
          <div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {(todaysTasks.length > 0 || todaysProjectTasks.length > 0) && (
            <div className={`px-6 py-3 border-b ${isDark ? 'border-white/5' : 'border-slate-50'}`}>
              ...
              <div className="space-y-1">
                ...
                {todaysProjectTasks.map(task => (
                  <div key={task.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50/50'}`}>
                    ...
                    {onRemoveProjectTaskFromToday && (
                      <button aria-label="Remove from Today" onClick={() => onRemoveProjectTaskFromToday(task.id)} className={`p-2 rounded -mr-1 ${isDark ? 'text-gray-600 hover:text-red-400' : 'text-slate-400 hover:text-red-500'}`}>
                        <Minus size={14} />
                      </button>
                    )}
                  </div>
                ...
                {manualTasks.map(task => (
                  <div key={task.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg ${isDark ? 'bg-violet-500/5' : 'bg-violet-50/50'}`}>
                    <Plus size={12} className={isDark ? 'text-violet-400' : 'text-violet-500'} />
                    <span className={`text-sm flex-1 truncate ${isDark ? 'text-gray-300' : 'text-slate-700'}`}>{task.title}</span>
                    <button aria-label="Remove from Today" onClick={() => onRemoveFromToday(task.id)} className={`p-2 rounded -mr-1 ${isDark ? 'text-gray-600 hover:text-red-400' : 'text-slate-400 hover:text-red-500'}`}>
                      <Minus size={14} />
                    </button>
                  </div>
                ))}
              ...
              {filteredSuggested.map(task => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {task.dueDate && (
                      <span className={`text-xs flex items-center gap-1 ${isDark ? 'text-gray-600' : 'text-slate-400'}`}>
                        <CalendarDays size={9} />
        ...
        {/* Footer */}
        <div className={`flex items-center justify-between px-6 py-3 border-t flex-shrink-0 ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
